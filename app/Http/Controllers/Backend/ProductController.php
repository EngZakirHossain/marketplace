...
class ProductController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $products = $query->paginate()->appends($query_param);
        ...
        return view('admin.pages.product.list', compact('products','search'));
        // return view('admin.pages.product.list', compact('search'));
    }
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        return (new FastExcel($storage))->export('products.xlsx');
    }

    public function limited_stock(Request $request)
    {
        $stock_limit = Setting::where('key','minimum_stock_limit')->first()->value;
        $query_param = [];
        $search = $request['search'];
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
    ...
    public function update_quantity(Request $request)
    {
        ...
        if ($stock_count >= 0) {
            ...
            $product->save();
            Toastr::success('product_quantity_updated_successfully!');
            return back();
        } else {
            Toastr::warning('product_quantity_can_not_be_less_than_0_!');
            return back();
        }
    }
